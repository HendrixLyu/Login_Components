...
async function register(req, res) {
  ...
  const Users = await axios
    .get(process.env.JSON_SERVER_PATH + "/user") //去数据库拿取user存的数据
  ...
  if (Users && Users.length) {
    ...
    Users.some(DB_User => {
      if (DB_User.name && DB_User.name === req.body.username) {
        //如果数据库存的DB_User.name = 前端传来的username
        res.status(200).send({
          msg: "Already have the username",
          code: "1",
          "Existing_username is:": req.body.username,
        }); //返回给前端的状态码+信息 //用户名存在
  ...
  if (!req.body.password) {
    ...
    //数据没问题,处理后存入数据库
    const getBcrypt = await axios.get(
      process.env.JSON_SERVER_PATH + "/bcrypt/1"
    ); //去数据库取bcrypt
    ...
    await axios.post(process.env.JSON_SERVER_PATH + "/user", {
      //将前端传入的username和加盐后的密码传入数据库(json-web_server)
